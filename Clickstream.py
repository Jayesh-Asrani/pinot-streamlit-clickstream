...
def event_ranking():
    ...
    selected_metric = st.selectbox("Select Metric", df_metric_name['metric_name'], index=14,
...
def session_analysis():
    ...
    curs.execute(query)
    df_metric_name = pd.DataFrame(curs, columns=[item[0] for item in curs.description])
    selected_metric = st.selectbox("Select Metric", df_metric_name['metric_name'], index=33,
                                   on_change=select_metric_callback, key='select_metric')
